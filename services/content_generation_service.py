...
class ContentGenerationService:
    ...
    def __init__(
        ...
        self.editorial_writer = EditorialWritingService()
    ...
    def _facebook_output(self, package, source, variant="standard"):
        ...
        editorial = self._editorial_social_copy(package, source, "facebook")
        if editorial:
            return editorial
    ...
    def _instagram_output(self, package, source, variant="standard"):
        ...
        editorial = self._editorial_social_copy(package, source, "instagram")
        if editorial:
            return editorial
    ...
    def _editorial_social_copy(self, package, source, platform):
        ...
        title = package.get("title") or package.get("headline") or package.get("story_title") or "MFR update"
        fact_sheet = self.editorial_writer.topic_fact_sheet(
            topic=title,
            current_relevance=package.get("why_today_matters", ""),
            historical=package.get("historical_mfr_evidence", {}),
            media=media,
            known_facts=[
                title,
                package.get("why_today_matters", ""),
                package.get("content_angle", ""),
                package.get("editorial_angle", ""),
                media_text
            ],
        ...
        copy = self.editorial_writer.generate_from_fact_sheet(
            fact_sheet,
            option={
                "title": title,
                "topic": package.get("opportunity_type", ""),
        ...
        text = copy.get("facebook" if platform == "facebook" else "instagram", "")
        if not text or not quality.get("passed"):
            return {}
        hashtags = copy.get(
            "facebook_hashtags" if platform == "facebook" else "instagram_hashtags",
            []
        )
        return {
            "title": "Facebook Post" if platform == "facebook" else "Instagram Caption",
            "copy_text": text,
            "hashtags": hashtags,
            "cta": self._profile_cta(
                package.get("suggested_cta", ""),
            ...
            "notes": self._notes_with_voice(
                "Generated by MFR Editorial Writing Engine.",
                platform=platform,
                source=source
            ),
            "editorial_writing": {
    ...
    def _short_version(self, recommendation, media, terms, profile, context_snapshot):
        ...
        if self._media_match_score(recommendation, media) < 2:
            return (
                ...
                "Use only the confirmed local details in the selected media "
                "before turning this into public copy."
            )
    ...
    def _platform_hashtags(self, recommendation, media, platform):
        ...
        tags = [
            tag for tag in self._unique(tags)
            if tag not in ("#MordenFireRescue", "#Morden")
        ]
        tags = [
            tag for tag in tags
            if tag != "#MordenMB"
