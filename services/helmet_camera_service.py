...
class HelmetCameraService:
    ...
    def analyze_video(self, media_id, video_path=None):
        ...
        try:
            ...
            pass
        samples = self._sample_technical_frames(path, metadata)
    ...
    def export_segment(
        ...
        command = [
            ...
            "-c",
            "copy" if preserve_audio else "copy",
            str(target)
        ]
    ...
    def reel_package(self, media_id, segment):
        ...
        caption = (
            "A short behind-the-scenes look at fire-service training and "
            "operations from Morden Fire & Rescue. Final review is required "
        ...
        return {
            ...
            "trimming_notes": segment.get("reason_selected", ""),
            "risk_warnings": risks,
    ...
    def _sample_technical_frames(self, path, metadata):
    ...
    def _candidate_segments(self, media_id, path, metadata, samples):
        ...
        for sample in samples:
            ...
            risk_penalty = 35 if risks else 0
            reel_score = max(0, min(100, technical - risk_penalty))
            ...
            ranked.append({
                ...
                "audio_score": 0,
                "risk_level": "manual_review" if risks else "low",
                "risk_flags": risks,
                "reason_selected": self._reason(
                    motion_score,
                    clarity_score,
                    exposure_score,
                    risks
                ...
                "visual_summary": (
                    "Technically selected helmet-camera segment based on "
                    "motion, clarity, exposure, and obstruction estimates."
                ),
                "cover_frame_seconds": round(center, 3),
                "status": "manual_review_only" if risks else "candidate",
                "analysis_version": self.ANALYSIS_VERSION,
    ...
    def _risk_flags(self, sample):
        ...
        if float(sample.get("blur") or 0) < 35:
            flags.append("blur_or_unstable_view")
    ...
    def _reason(self, motion, clarity, exposure, risks):
        ...
        if risks:
            parts.append("manual review required due to technical risk flags")
